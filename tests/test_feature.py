...
class FeatureTestCase(unittest.TestCase):
    ...
    def test_feature_polygons_are_closed_with_at_least_four_coordinates(self):
        ...
        self.assertListEqual(
            self.feature(topology, topology['objects']['foo'])['geometry']['coordinates'],
            [[[0, 0], [1, 1], [0, 0], [0, 0]]]
        )
        self.assertListEqual(
            self.feature(topology, topology['objects']['bar'])['geometry']['coordinates'],
            [[[0, 0], [1, 1], [0, 0], [0, 0]]]
        )

    def test_feature_top_level_geometry_collections_are_mapped_to_feature_collections(self):
        t = self.simple_topology({
    ...
    def test_feature_top_level_geometry_collections_do_not_have_ids_but_second_level_geometry_collections_can(self):
        t = self.simple_topology({
            "type": "GeometryCollection",
            "id": "collection",
            "geometries": [{
    ...
    def test_feature_top_level_geometry_collections_do_not_have_properties_but_second_level_geometry_collections_can(self):
        t = self.simple_topology({
            "type": "GeometryCollection",
            "properties": {
                "collection": True
    ...
    def test_feature_the_object_id_is_optional(self):
        ...
        self.assertEqual(
            self.feature(t, t['objects']['foo']).get('id', None), 
            None
        )
    ...
    def test_feature_when_multiple_arc_indexes_are_specified_coordinates_are_stitched_together(self):
        t = self.simple_topology({
            "type": "LineString",
            "arcs": [1, 2]
        })
